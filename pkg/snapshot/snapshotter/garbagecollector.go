		...
		case <-time.After(ssr.config.garbageCollectionPeriodSeconds * time.Second):
			...
			case GarbageCollectionPolicyExponential:
				...
				for snapStreamIndex := len(snapStreamIndexList) - 1; snapStreamIndex > 0; snapStreamIndex-- {
					...
					deletedSnap, err := ssr.garbageCollectDeltaSnapshots(ssr.ctx, snapList[snapStreamIndexList[snapStreamIndex-1]:snapStreamIndexList[snapStreamIndex]])
					total += deletedSnap
					if err != nil {
						continue
					}
					...
					if deleteSnap {
						...
						if err := ssr.config.store.Delete(ssr.ctx, *nextSnap); err != nil {
						...
						total++
						garbageCollectChunks(ssr.ctx, ssr.config.store, snapList, snapStreamIndexList[snapStreamIndex-1]+1, snapStreamIndexList[snapStreamIndex])
					}
				}
			...
			case GarbageCollectionPolicyLimitBased:
				...
				for snapStreamIndex := 0; snapStreamIndex < len(snapStreamIndexList)-1; snapStreamIndex++ {
					deletedSnap, err := ssr.garbageCollectDeltaSnapshots(ssr.ctx, snapList[snapStreamIndexList[snapStreamIndex]:snapStreamIndexList[snapStreamIndex+1]])
					total += deletedSnap
					if err != nil {
						continue
					}
					if snapStreamIndex < len(snapStreamIndexList)-ssr.config.maxBackups {
						...
						if err := ssr.config.store.Delete(ssr.ctx, *snap); err != nil {
						...
						metrics.GCSnapshotCounter.With(prometheus.Labels{metrics.LabelKind: snapstore.SnapshotKindFull, metrics.LabelSucceeded: metrics.ValueSucceededTrue}).Inc()
						total++
						garbageCollectChunks(ssr.ctx, ssr.config.store, snapList, snapStreamIndexList[snapStreamIndex]+1, snapStreamIndexList[snapStreamIndex+1])
					}
				}
			}
...
func (ssr *Snapshotter) garbageCollectDeltaSnapshots(ctx context.Context, snapStream snapstore.SnapList) (int, error) {
	...
	for i := len(snapStream) - 1; i > 0; i-- {
		...
		if err := ssr.config.store.Delete(ctx, *snapStream[i]); err != nil {
			ssr.logger.Warnf("GC: Failed to delete snapshot %s: %v", snapPath, err)
