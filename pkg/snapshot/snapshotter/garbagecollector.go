		...
		case <-time.After(ssr.config.garbageCollectionPeriodSeconds * time.Second):
			...
			case GarbageCollectionPolicyExponential:
				...
				for snapStreamIndex := len(snapStreamIndexList) - 1; snapStreamIndex > 0; snapStreamIndex-- {
					...
					deletedSnapshots, err := ssr.garbageCollectDeltaSnapshots(snapList[snapStreamIndexList[snapStreamIndex-1]:snapStreamIndexList[snapStreamIndex]])
					total += deletedSnapshots
					if err != nil {
						continue
					}
			...
			case GarbageCollectionPolicyLimitBased:
				...
				for snapStreamIndex := 0; snapStreamIndex < len(snapStreamIndexList)-1; snapStreamIndex++ {
					deletedSnapshots, err := ssr.garbageCollectDeltaSnapshots(snapList[snapStreamIndexList[snapStreamIndex]:snapStreamIndexList[snapStreamIndex+1]])
					total += deletedSnapshots
					if err != nil {
						continue
					}
